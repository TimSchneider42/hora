...
class AllegroHandHora(VecTask):
    def __init__(self, config, device_id, render_device_id, headless):
        ...
        self.env_timeout_counter = (
            to_torch(np.zeros((len(self.envs)))).long().to(self.device)
        )  # max 10 (10000 envs)
    ...
    def compute_reward(self, actions):
        ...
        rotate_reward = torch.clip(
            vec_dot, max=self.angvel_clip_max, min=self.angvel_clip_min
        )
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                ...
                # Rotation axis
                r = self.rot_axis_buf[i].cpu().numpy() * 0.08
                self.gym.add_lines(
                    self.viewer,
                    self.envs[i],
                    1,
                    [p0[0], p0[1], p0[2], p0[0] + r[0], p0[1] + r[1], p0[2] + r[2]],
                    [1, 0.1, 1],
                )
